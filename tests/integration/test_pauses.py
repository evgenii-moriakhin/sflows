...
async def test_workflow_pause_but_it_is_not_paused_cause_almost_complete(
    task_group: TaskGroup,
):
    activity_executed = 0
    ...
    try:
        ...
        assert await manager.await_task_completion_and_get_validated_result(task) == "success"
        resumed, msg = await manager.resume_task(task)
        assert not resumed
        assert re.match(r"Cannot retry task .* because it is not paused", msg)
...
async def test_parallel_resume_tries(task_group: TaskGroup):
    first_stage_executed = 0
    second_stage_executed = 0
    ...
    try:
        ...
        async def resume_and_store_result(results: list[bool] = results):
            results.append((await manager.resume_task(task))[0])

        task_group.start_soon(resume_and_store_result)
        task_group.start_soon(resume_and_store_result)
        task_group.start_soon(resume_and_store_result)
        await anyio.sleep(3)

        resume_tries = collections.Counter(results)
        assert resume_tries[True] == 1
        assert resume_tries[False] == 2
